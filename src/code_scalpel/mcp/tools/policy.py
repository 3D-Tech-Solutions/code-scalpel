...
async def validate_paths(
    ...
    **Returns:**
    ...
    try:
        # [20260311_BUGFIX] Validate path-request shape and normalize project roots before helper dispatch.
        if not paths:
...
async def verify_policy_integrity(
    ...
    **Args:**
        ...
        manifest_source (str): Where to load manifest from. Default: "file". Options: "file", "git", "env".
    ...
    **Returns:**
    ...
    started = time.perf_counter()
    try:
        # [20260311_BUGFIX] Validate policy verification shape and normalize paths before helper dispatch.
...
async def code_policy_check(
    ...
    **Returns:**
    ...
    started = time.perf_counter()
    try:
        # [20260311_BUGFIX] Validate policy-check shape and normalize paths before helper dispatch.
        if not paths:
            duration_ms = int((time.perf_counter() - started) * 1000)
            tier = _get_current_tier()
            return make_envelope(
        ...
        if compliance_standards and tier != "enterprise":
            # [20260310_BUGFIX] Fail fast instead of silently downgrading when
            # callers request Enterprise-only compliance auditing.
            message = (
        ...
        if generate_report and tier != "enterprise":
            # [20260310_BUGFIX] Report generation is part of the Enterprise
            # compliance workflow and must not degrade silently.
