    ...
    **Returns:**
    ...
    try:
        ...
        if code is None and file_path is not None:
            ...
            try:
                # [20260311_BUGFIX] Route file-based analysis through the shared path
                # resolver so malformed Windows/WSL paths like '/k:/...' can recover.
        ...
        # [20260327_FEATURE] Emit telemetry event
        telemetry.emit_tool_event(
            ...
            output_summary={
                ...
                "function_count": len(result.functions) if result.functions else 0,
                "class_count": len(result.classes) if result.classes else 0,
        ...
        # [20260327_FEATURE] Emit telemetry event on error
        telemetry.emit_tool_event(
            tool_name="analyze_code",
            tier_applied=tier,
            duration_ms=float(duration_ms),
