...
class RustVisitor(TreeSitterVisitor):
    ...
    def visit_closure_expression(self, node: Any) -> IRFunctionDef:
                    ...
                    if p.type in ("identifier", "closure_parameter", "parameter"):
                        ...
                        if p.type == "identifier":
                            ...
                            if id_child is None:
                                ...
                                nc = [c for c in p.named_children if c.type == "identifier"]
        ...
        for child in node.named_children:
            if child.type == "block":
                ...
                break
            elif child.type not in ("closure_parameters", "type_identifier",
                                     "primitive_type", "generic_type",
    ...
    def visit_assignment_expression(self, node: Any) -> IRAssign:
        ...
        return IRAssign(targets=[], value=IRConstant(value=None), loc=self._get_location(node))
    ...
    def visit_integer_literal(self, node: Any) -> IRConstant:
        ...
        try:
            return IRConstant(value=int(text.replace("_", "").rstrip("uUiI0123456789").rstrip("uUiI") or text), loc=self._get_location(node))
        except ValueError:
    ...
    def visit_float_literal(self, node: Any) -> IRConstant:
        text = self._get_text(node)
        try:
            return IRConstant(value=float(text.replace("_", "").rstrip("f3264")), loc=self._get_location(node))
        except ValueError:
            return IRConstant(value=text, loc=self._get_location(node))
