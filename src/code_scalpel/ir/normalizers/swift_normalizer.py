...
class SwiftVisitor(TreeSitterVisitor):
    ...
    def visit_import_declaration(self, node: Any) -> IRImport:
        ...
        if not parts:
            ...
            if root and root.named_children:
                parts = [self._get_text(ch) for ch in root.named_children if ch.is_named]
        ...
        for ch in named:
            ...
            if not found_body and not past_else:
                if ch.type == "statements":
                    ...
                    found_body = True
                elif ch.type not in {"comparison_expression", "conjunction_expression",
                                     "disjunction_expression", "boolean_literal",
    ...
    def visit_for_statement(self, node: Any) -> IRFor:
        ...
        for i, ch in enumerate(named):
            ...
            if ch.type == "statements":
                body = self._visit_stmts(ch)
            elif iter_ is not None and isinstance(iter_, IRName) and iter_.id == "<unknown>":
    ...
    def _collect_lambda_params(self, node: Any) -> List[IRParameter]:
        ...
        children_list = [c for c in target.named_children if c.type == "simple_identifier"]
